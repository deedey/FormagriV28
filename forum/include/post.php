...
function check_data($host, $author, $subject, $body, $email) 
{
	...
	if (!empty ($email)) 
	{
		if (!preg_match("/.+@.+\\..+/i", $email) && $email != $Password && $email != $ModPass) {
			if ($email_reply) 
			{
				$IsError = $lNoEmail;
			}
...
function censor($author, $subject, $email, $body) 
{
	...
	if (is_array($profan)) 
	{
		...
		while (list (, $sWord) = each($profan)) 
		{
			...
			{
				if (strtoupper($author) == strtoupper($sWord))
					$author = $blurb;
				$author = preg_replace("/^$sWord([^a-zA-Z])/i", "$blurb\\1", $author);
				$author = preg_replace("/([^a-zA-Z])$sWord$/i", "\\1$blurb", $author);
				while (preg_match("/([^a-zA-Z])($sWord)([^a-zA-Z])/i", $author))
				{
					$author = preg_replace("/([^a-zA-Z])($sWord)([^a-zA-Z])/i", "\\1$blurb\\3", $author);
				}
			}
			if (strstr(strtoupper($subject), strtoupper($sWord))) 
			{
				if (strtoupper($subject) == strtoupper($sWord))
					$subject = $blurb;
				$subject = preg_replace("/^$sWord([^a-zA-Z])/i", "$blurb\\1", $subject);
				$subject = preg_replace("/([^a-zA-Z])$sWord$/i", "\\1$blurb", $subject);
				while (preg_match("/([^a-zA-Z])($sWord)([^a-zA-Z])/i", $subject))
				{
					$subject = preg_replace("/([^a-zA-Z])($sWord)([^a-zA-Z])/i", "\\1$blurb\\3", $subject);
				}
			}
			if (strstr(strtoupper($email), strtoupper($sWord))) 
			{
				if (strtoupper($email) == strtoupper($sWord))
					$email = "";
				$email = preg_replace("/^$sWord([^a-zA-Z])/i", "$blurb\\1", $email);
				$email = preg_replace("/([^a-zA-Z])$sWord$/i", "\\1$blurb", $email);
				while (preg_match("/([^a-zA-Z])($sWord)([^a-zA-Z])/i", $email))
				{
					$email = preg_replace("/([^a-zA-Z])($sWord)([^a-zA-Z])/i", "\\1$blurb\\3", $email);
				}
			}
			if (strstr(strtoupper($body), strtoupper($sWord))) 
			{
				if (strtoupper($body) == strtoupper($sWord))
					$body = $blurb;
				$body = preg_replace("/^$sWord([^a-zA-Z])/i", "$blurb\\1", $body);
				$body = preg_replace("/([^a-zA-Z])$sWord$/i", "\\1$blurb", $body);
				while (preg_match("/([^a-zA-Z])($sWord)([^a-zA-Z])/i", $body))
				{
					$body = preg_replace("/([^a-zA-Z])($sWord)([^a-zA-Z])/i", "\\1$blurb\\3", $body);
				}
			}
		}
	}
...
function post_to_database() 
{
	...
	$req = mysql_query("select max(id) from $ForumTableName");
	...
	if ($PhorumMail && ($thread == 0)) 
	{
		...
		preg_match('/^[[:space:]]*(\[[^]]+\][[:space:]]+)?((re|aw)(\[[[:digit:]]+\])?:[[:space:]]+)*(.+)$/i', $subject, $threadsubj);
...
*/
	...
	$sSQL = "Insert Into $ForumTableName (id, author, email, datestamp, subject, host, thread, parent, email_reply, attachment, approved, msgid) values ('$id', '$author', '$email', '$datestamp', '$subject', '$ip', '$thread', '$parent', '$email_reply', '$org_attachment', '$approved', '$msgid')";
	$q->query($DB, $sSQL);
	if (!$q->result) 
	{
		return ($q->error() . "<br>$sSQL");
