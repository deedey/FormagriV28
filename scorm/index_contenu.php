<?php
/*
modification à partir du 15/11/2006
Gestion de la visibilité des items
*/
if (!isset($_SESSION)) session_start();
//echo '<div id="mydiv" style="border:1px solid silver;">  </div>';
//echo $content;
...
$Ext = '_'.$numero_groupe;
   ...
   <SCRIPT language=javascript1.2>
        ...
        function Recharger() {
             parent.frames['contenu'].location.href= "vide.php?titre=<?php echo $msq_tit_label;?>&contenu=<?php echo $mess_quitAct;?>";
        }
   </SCRIPT>
   <?php
}
...
if ($_SESSION['typ_user'] == 'APPRENANT') $cours = 1;
if ($cours == 1)
{
   ...
   if ($ecran == '' || !isset($ecran) || $ecran == 'normal')
      echo "<A HREF=\"javascript:parent.index_contenu.location.href='index_contenu.php?id_parc=$id_parc&id_seq=$id_seq&scormid=$scormid&actuelle=$actuelle&cours=1&numero_groupe=$numero_groupe&ecran_new=large'\" onclick=\"javascript:Frame_fermer();\" title=\"$mess_agecr\">$img_ecran</A><BR>";
   else
      echo "<A HREF=\"javascript:parent.index_contenu.location.href='index_contenu.php?id_parc=$id_parc&id_seq=$id_seq&scormid=$scormid&actuelle=$actuelle&cours=1&numero_groupe=$numero_groupe&ecran_new=normal'\" onclick=\"javascript:Frame_ouvrir();\" title=\"$mess_affmenu\">$img_ecran1</A><BR>";
}
//$largeur = ($cours == 1) ? '70%': '100%';
$affiche_entete = "<CENTER><TABLE bgColor='#298CA0' cellspacing='2' cellpadding='0' width=95%><TR><TD>";
$affiche_entete .= "<TABLE bgColor='#FFFFFF' cellspacing='1' cellpadding='0' width='100%' border=0>";
$affiche_entete .= "<TR><TD background=\"../images/fond_titre_table.jpg\" colspan='3' height='34' align='center' valign='center'><Font size='3' color='#FFFFFF'><B>$mess_cours</B></FONT>";
...
$ordrepere = GetDataField ($connect,"SELECT mod_pere_lb
                               FROM scorm_module
                               WHERE mod_cdn = '$scormid' AND
                               mod_seq_no ='$id_seq'","mod_pere_lb");
...
if ($req_seq1 == TRUE)
{
     $scopere = GetDataField ($connect,"SELECT mod_cdn
                               FROM scorm_module
                               WHERE mod_numero_lb = \"$ordrepere\" AND
                               mod_content_type_lb='LABEL' AND
                               mod_seq_no ='$id_seq'","mod_cdn");
  ...
  if ($cours == 1)
  {
     ...
     if ($j == $i)
     {
        $sql2 = "UPDATE scorm_util_module$Ext SET
                ...
                `entry` = 'RESUME',
                `credit` = 'CREDIT',
                `session_time` = '0000:00:00.00' WHERE
                `user_module_no` = '$id_user' AND
                `mod_module_no` ='$scopere'";
        $requete = mysql_query($sql2);
        if ($cours == 1)
           $requete_seq = mysql_query("UPDATE suivi2$Ext set suiv2_etat_lb ='TERMINE' WHERE
                                       suiv2_seq_no = '$id_seq' AND
...
{     $sco['espacer'][$i] = '';
   ...
   if ($cours == 1)
   {
      ...
      elseif($nb_enfants == 0 && $prochain_niveau == $sco['niveau_mod'][$i])
          $ajoutULD = "\n</li><li class=\"treeItem\">\n";

      if ($prochain_niveau+1 == $sco['niveau_mod'][$i])
          $ajoutULD .= "\n</li>\n</ul>\n<li class=\"treeItem\">\n";
      elseif ($prochain_niveau+2 == $sco['niveau_mod'][$i])
          $ajoutULD = "\n</li>\n</ul>\n</li></li>\n</ul>\n<li class=\"treeItem\">\n";
      elseif ($prochain_niveau+3 == $sco['niveau_mod'][$i])
          $ajoutULD = "\n</li>\n</ul>\n</li>\n</ul>\n</li></li>\n</ul>\n<li class=\"treeItem\">\n";
...
//          $ajoutULF = "\n</ul>\n</li>\n<li class=\"treeItem\">\n";
      ...
      if (strstr($type_sequence,"SCORM") && !strstr($type_sequence,"AICC"))
      {
         ...
         if ($sco['datafromlms'][$i] != '')
            $launch .="?".$sco['datafromlms'][$i];
         $launcher = urlencode($launch);
         $suitable = "";
      }
      elseif (strstr($type_sequence,"SCORM") && strstr($type_sequence,"AICC"))
      {
         ...
         elseif (strstr($sco['url_mod'][$i],"http://"))
            $launch = $sco['url_mod'][$i];
         $session_lms = md5($login)."|".$sco['id_mod'][$i]."|$id_seq|$id_parc|$id_user|$numero_groupe";
      ...
      if (strstr($agent,"MSIE"))
         $interligne = ($nb_mod > 15)? "" : "<BR>";
      else
         $interligne = ($nb_mod > 15)? "<BR>" : "<BR><BR>";
      if ((($cours == 1 && !$utilisateur) || $utilisateur > 0) && $launcher != "" && ((isset($noLink) && $noLink != 1) || !isset($noLink)))
      {
         $scormid = $sco['id_mod'][$i];
         $lien_index = urlencode("index_contenu.php?id_parc=$id_parc&id_seq=$id_seq&cours=$cours&scormid=".$sco['id_mod'][$i]."&numero_groupe=$numero_groupe&actuelle=".$sco['id_mod'][$i]);
         $affiche_index .= "$ajoutULF $div<A HREF=\"javascript:void(0);\" ".
                "onClick=\"javascript:parent.location.replace('lancer_sco.php?lien=$launcher&lien_index=$lien_index&cours=$cours&id_seq=$id_seq&numero_groupe=$numero_groupe&id_parc=$id_parc&scormid=".$sco['id_mod'][$i]."&$suitable');\" title=\"$alter\">".
                $etat_img."&nbsp;".$nom1."</A></span>$ajoutULD";
      }
      elseif ($cours == 1 && (($launcher == ""  && ((isset($noLink) && $noLink != 1) || !isset($noLink))) || $utilisateur > 0))
         $affiche_index .= "$ajoutULF".$etat_img."&nbsp;"."<B><acronym title=\"".$sco['titre_mod'][$i]."\">".$nom1."</acronym></B>$ajoutULD";
      elseif ($cours == 1 && $noLink == 1)
         $affiche_index .= $ajoutULF.$etat_img."&nbsp;"."<B>&nbsp;<acronym title=\"$msq_act_prereq : $titre_prereq\">".$nom1."</acronym></B>$ajoutULD";
      elseif ($cours != 1 || !isset($cours))
      {
       if ($sco['parent_mod'][$i] != $ordre || ($sco['parent_mod'][$i] == $ordre && (($id_dvlp == $sco['id_mod'][$i] && $dvlp == 1) || !isset($id_dvlp) || $dvlp == 0 )))
       {
          if ($sco['url_mod'][$i] != '' && $actuelle != $sco['id_mod'][$i])
            $affiche_index .= "$ajoutULF $div<A href=\"".urldecode($launcher)."\" title = \"".$sco['titre_mod'][$i]."\" target='contenu' ".
            "onclick = \"javascript:document.location.replace('index_contenu.php?id_seq=$id_seq&id_parc=$id_parc&actuelle=".$sco['id_mod'][$i]."&dvlp=$dvlp&id_dvlp=$id_dvlp&ordre=$ordre');\">".
            $nom1."</A></span>$ajoutULD";
          elseif ($sco['url_mod'][$i] != '' && $actuelle == $sco['id_mod'][$i])
            $affiche_index .= "$ajoutULF $div $nom1 <A HREF=\"javascript:void(0);\" title = >"."</A></span>$ajoutULD";
          else
          {
             if (!isset($id_dvlp) || ($dvlp == 0 && $id_dvlp == $sco['id_mod'][$i]) || $id_dvlp != $sco['id_mod'][$i])
              $affiche_index .= "$ajoutULF"."&nbsp;<B><acronym title=\"".$sco['titre_mod'][$i]."\">".$nom1."</acronym></B>$ajoutULD ";
...
if ($nb_items > 1)
   if ($sco['niveau_mod'][$nb_items-1] > 1)
   {
    ...
    if ($cours == 1)
    {
      ...
      $req_nav = mysql_query("SELECT mod_cdn,mod_launch_lb,mod_prereq_lb,mod_datafromlms,mod_visible FROM scorm_module WHERE mod_launch_lb != '' AND mod_seq_no = '$id_seq' AND mod_visible='TRUE' ORDER by mod_cdn");
      $nbr_sconav = mysql_num_rows($req_nav);
         ...
         while ($i < $nbr_sconav)
         {
           ...
           if ($scorm_actuel == $sconavid && $i < $nbr_sconav-1 && $affiche_suivant == 0){
              ...
              elseif (strstr($Suiv_lch,"http://"))
                 $Suiv_lch = $Suiv_lch;
              if (strstr($type_sequence,"AICC"))
              {
                 $session_lms = md5($login)."|".$sco['id_mod'][$i]."|$id_seq|$id_parc|$id_user|$numero_groupe";
                 $suitable = "aicc_sid=$session_lms&aicc_url=".urlencode("$adresse_http/scorm/aicc.php")."&vendor_param=".$Prec_DFLMS;
              }
              else
              {
                 if ($Suiv_DFLMS != '')
         ...
         $navigation = '';
         if (isset($ScoPrec) && $ScoPrec != '')
         {
            $lien_index = urlencode("index_contenu.php?id_parc=$id_parc&id_seq=$id_seq&cours=$cours&numero_groupe=$numero_groupe&scormid=$ScoPrec&actuelle=$ScoPrec");
            $navigation .= "<A HREF=\"javascript:void(0);\" ".
                "onClick=\"javascript:parent.location.replace('lancer_sco.php?lien=".urlencode($Prec_lch)."&lien_index=$lien_index&cours=$cours&numero_groupe=$numero_groupe&id_seq=$id_seq&id_parc=$id_parc&actuelle=$ScoPrec&scormid=$ScoPrec&$suitable');\" title=\"$mess_prec\">".
                "<IMG SRC='../images/precedent.gif' border='0' Alt=\"$mess_prec\"></A>";
         }
         if (isset($ScoSuiv) && $ScoSuiv != '')
         {
            $lien_index = urlencode("index_contenu.php?id_parc=$id_parc&id_seq=$id_seq&cours=$cours&numero_groupe=$numero_groupe&scormid=$ScoSuiv&actuelle=$ScoSuiv");
            $navigation .= "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<A HREF=\"javascript:void(0);\" ".
                "onClick=\"javascript:parent.location.replace('lancer_sco.php?lien=".urlencode($Suiv_lch)."&lien_index=$lien_index&cours=$cours&id_seq=$id_seq&id_parc=$id_parc&numero_groupe=$numero_groupe&actuelle=$ScoSuiv&scormid=$ScoSuiv&$suitable');\" title=\"$mess_suiv\">".
