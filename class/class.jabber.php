...
class Jabber
{
        ...
        function Jabber()  {
           GLOBAL $JABBER;
                $this->server                                = $JABBER->server;
                $this->port                                  = $JABBER->port;

                $this->username                                = $JABBER->username;
                $this->password                                = $JABBER->password;
                $this->resource                                = NULL;
                ...
                $this->error_codes                        = array(400 => "Mauvaise requete",
                                                                                        401 => "Non autorise",
                                                                                        402 => "Payment Requis",
                                                                                        403 => "Interdit",
                                                                                        404 => "Introuvable",
                                                                                        405 => "Non defini",
                                                                                        406 => "Inacceptable",
                                                                                        407 => "Enegistrement Requis",
                                                                                        408 => "Temps de requete depasse",
                                                                                        409 => "Conflit",
                                                                                        500 => "Erreur interne du serveur",
                                                                                        501 => "Non implemente",
                                                                                        502 => "Erreur de suppression du serveur",
                                                                                        503 => "Service non valable",
                                                                                        504 => "Temps depasse pour la suppression",
                                                                                        510 => "Deconnecte");
        }
        ...
        function Connect()
        {
                ...
                if ($this->CONNECTOR->OpenSocket($this->server, $this->port))
                {
                        ...
                        {
                                $this->AddToLog("ERREUR: Connexion() #1");
                                return FALSE;
                        }
                }
                else
                {
                        $this->AddToLog("ERREUR: Connexion() #2");
                        return FALSE;
                }
        }
        ...
        function SendAuth()
        {
                ...
                if ($this->GetInfoFromIqType($packet) == 'result' && $this->GetInfoFromIqId($packet) == $this->auth_id)
                {
                        ...
                        {
                                $this->AddToLog("ATTENTION: SendAuth() - mhash() n'est pas valable; screw 0k and digest method, we need to go with plaintext auth");
                        }
        ...
        function SendMessage($to, $type = "normal", $id = NULL, $content = NULL, $payload = NULL)
        {
                if ($to && is_array($content))
                {
                        ...
                        $content = $this->_array_htmlspecialchars($content,ENT_QUOTES,'ISO-8859-1');
        ...
        function RosterAddUser($jid = NULL, $id = NULL, $name = NULL)
        {
                ...
                if ($jid)
                {
                        ...
                        $payload .= ($name) ? " name='" . htmlspecialchars($name,ENT_QUOTES,'ISO-8859-1') . "'" : '';
                        $payload .= "/>\n";
        ...
        function SendIq($to = NULL, $type = 'get', $id = NULL, $xmlns = NULL, $payload = NULL, $from = NULL)
        {
                ...
                elseif ($id && $xmlns)
                {
                        ...
                        $xml .= ($to) ? " to='" . htmlspecialchars($to,ENT_QUOTES,'ISO-8859-1') . "'" : '';
                        $xml .= ($from) ? " from='$from'" : '';
                        $xml .= ">
                                                <query xmlns='$xmlns'>
                                                        $payload
        ...
        function _create_logfile()
        {
                ...
                {
                        echo$this->log_filename; $this->log_filehandler = fopen($this->log_filename, 'w');
                }
        }
        ...
        function AddToLog($string)
        {
                if ($this->enable_logging)
                {
                        ...
                        {
                                $this->log_array[] = htmlspecialchars($string,ENT_QUOTES,'ISO-8859-1');
                        }
                }
        }
...
class MakeXML extends Jabber
{
        ...
        function AddPacketDetails($string, $value = NULL)
        {
                ...
                echo "\$this->nodes$node = \"" . htmlspecialchars($value,ENT_QUOTES,'ISO-8859-1') . "\";<br/>";
                eval("\$this->nodes$node = \"" . htmlspecialchars($value,ENT_QUOTES,'ISO-8859-1') . "\";");
        }
                ...
                if (is_array($array))
                {
                        ...
                        foreach ($array as $key => $value)
                        {
                                if (is_array($value) && $key == "@")
                                {
                                        ...
                                        {
                                                $subvalue = htmlspecialchars($subvalue,ENT_QUOTES,'ISO-8859-1');
                                ...
                                elseif ($key == "#")
                                {
                                        $text .= htmlspecialchars($value,ENT_QUOTES,'ISO-8859-1');
                                }
