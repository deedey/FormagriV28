...
if ($export_parc == 1 && !$copier_parc)
{
  ...
  <TABLE bgColor=#FFFFFF cellspacing='0' cellpadding=5>
   ...
   <TR>
      ...
      <TD nowrap>
         <INPUT TYPE='TEXT'  name="mail_dest" size="40" value="<? echo $mail_envoi; ?>" align='middle'>
      </TD>
   </TR>
   <TR>
      <TD>
         <B><?php  echo "Adresse de la plate-forme destinatrice:<br />http://maplateforme.fr";?></B>
      </TD>
      <TD nowrap>
...
if ($export_parc == 1 && $copier_parc == 1)
{
  ...
  $parcours_sql = "ModuleExport_".$date_complement.".exp";
    ...
    if ($mail_dest != "")
    {
       ...
       {
           $msg .= "Pour importer le module, copiez le lien suivant :\n".
                   "$site/transfert_module.php?file=$adrExp\n".
                   "cliquez sur le bouton droit de la souris sur un lien quelconque de la plateforme\n ".
                   "après que vous vous soyez authentifié, et choisissez \"Ouvrir dans une autre fenêtre ou un autre onglet,\"\n".
                   "coller le lien dans la barre d'adresse et taper sur \"Entrée\".\nLe module sera importé dans votre plate-forme\n".
                   "Notez bien que certains hébergeurs gratuits tel que Free n'autorisent pas ce type de transfert.";
       }
       ...
       {
           $origine=$nom_user."  ".$typ_user;
           Connecter(str_replace("http://","",(str_replace(".educagri.fr","",$site))));
           $msg = $messg;
           ...
           $prenom_user = GetDataField ($connect,"select util_prenom_lb from utilisateur where util_cdn = $id_user","util_prenom_lb");
           $requete = mysql_query("INSERT INTO messagerie (envoyeur,origine,contenu,date,sujet,id_user) VALUES (\"$user_mail\",\"$subject\",\"$msg\",\"$date_messagerie\",\"$origine\",$user_mail)");
       }
