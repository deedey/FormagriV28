...
$today = date("d/n/Y");
  ...
  while ($i < $Nb_act_seq)
  {
     if ($scormOk == 1)
     {
        ...
        $sql = mysql_query("UPDATE scorm_util_module_$numero_groupe SET
                                 `lesson_location` = '',
                                 `lesson_mode` = 'NORMAL',
        ...
        $modif_seq = mysql_query("UPDATE suivi2_$numero_groupe SET suiv2_etat_lb = 'A FAIRE' WHERE
                                suiv2_seq_no = $id_seq AND
                                suiv2_utilisateur_no = $utilisateur");
        $modif_parc = mysql_query("UPDATE suivi3_$numero_groupe SET suiv3_etat_lb = 'A FAIRE' WHERE
     ...
     else
     {
       ...
       if ($id_ress == 0)
       {
          $suivi_etat= mysql_query ("update suivi1_$numero_groupe set suivi_etat_lb = 'PRESENTIEL',
       ...
       {
          $suivi_etat= mysql_query ("update suivi1_$numero_groupe set suivi_etat_lb = 'A FAIRE',
                                 suivi_fichier_lb= '',suivi_note_nb1 = '',
                                 suivi_commentaire_cmt='Activit&eacute; repass&eacute;e' where
                                 suivi_act_no= $id_act and
                                 suivi_utilisateur_no=$utilisateur");
       }

       $suivi_etat_sequence = GetDataField ($connect,"select suiv2_etat_lb from suivi2_$numero_groupe where
                                                  suiv2_seq_no = $id_seq and
                                                  suiv2_utilisateur_no=$utilisateur","suiv2_etat_lb");
       if ($suivi_etat_sequence != "A FAIRE" && $Nb_act_seq > 1)
       {
           $change_suivi_etat= mysql_query ("update suivi2_$numero_groupe set suiv2_etat_lb = 'A FAIRE' where
                                       suiv2_seq_no= $id_seq and
                                       suiv2_utilisateur_no=$utilisateur");
       }
       $suivi_etat_parcours = GetDataField ($connect,"select suiv3_etat_lb from suivi3_$numero_groupe where
                                                  suiv3_parc_no = $id_parc and
                                                  suiv3_utilisateur_no=$utilisateur","suiv3_etat_lb");
       if ($suivi_etat_parcours == "TERMINE")
       {
           $change_suivi_etat= mysql_query ("update suivi3_$numero_groupe set suiv3_etat_lb = 'EN COURS' where
                                       suiv3_parc_no= $id_parc and
                                       suiv3_utilisateur_no=$utilisateur");
       }
     }
   ...
   $prescripteur = GetDataField ($connect,"select presc_prescripteur_no from prescription_$numero_groupe where
   ...
   $formateur = GetDataField ($connect,"select presc_formateur_no from prescription_$numero_groupe where
                                        presc_seq_no = $id_seq and
