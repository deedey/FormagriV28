...
dbConnect();
$content = '';
$str1 = trimUltime(strip_tags(html_entity_decode(GetDataField ($connect,"select wkbody_body_cmt from wikibodies where wkbody_cdn = ".$_GET['actuId'],"wkbody_body_cmt"),ENT_QUOTES,'ISO-8859-1')));
$str2 = trimUltime(strip_tags(html_entity_decode(GetDataField ($connect,"select wkbody_body_cmt from wikibodies where wkbody_cdn = ".$_GET['IdCdn'],"wkbody_body_cmt"),ENT_QUOTES,'ISO-8859-1')));
...
$content .= '<div class="completBody"><span style="font-size:11px;font-weight:bold;">Instance en cours en date du '.
            date('d-m-Y à H\h i\'',$_GET['actuDt']).' par '.NomUser($_GET['actuUser']).'</span><br />'.$str1.'</div>';
$content .= '<div class="completBody"><span style="font-size:11px;font-weight:bold;">Comparée à l\'instance en date du '.
            date('d-m-Y à H\h i\'',$_GET['IdDt']).' par '.NomUser($_GET['IdUser']).'</span><br />'.$str2.'</div>';
...
$content .= '<span style="font-weight:normal;">Identique</span>'.
            '<span style="background-color:yellow;padding:0 4px 0 4px;margin-right:4px;">Ajouté</span>'.
            '<span style="text-decoration:line-through;background-color:red;color:#fff;padding:0 4px 0 4px;">Supprimé</span></div><p>';
