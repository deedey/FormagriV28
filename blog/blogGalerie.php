...
<link rel="stylesheet" type="text/css" href="<?php  echo $monURI;?>/blog/css/ppgallery.css"/>
...
<script type="text/javascript" src="<?php  echo $monURI;?>/blog/js/ppgallery.js"></script>
...
<?php
$content = '';
$reqImg = mysql_query("select * from blogmg,blogshare where bgimg_auteur_no=bgshr_auteur_no and bgshr_img_on=1 group by bgimg_content_blb");
...
while ($oImg = mysql_fetch_object($reqImg))
{
   ...
   $content .= '<li><a href="lib/affiche_image.php?provenance=paragraphe&numImg='.$oImg->bgimg_cdn.'&'.$NumPrg.'&'.$oImg->bgimg_cdn.'" style="margin-bottom:4px;">'.
              '<img src=lib/affiche_image.php?provenance=paragraphe&numImg='.$oImg->bgimg_cdn.' border="0" width="60" height="40"></a>';
   $content .= '<br /><span id="bout_msg" style="margin-top:4px;" onClick="'.
               'parent.$(\'#'.$_GET['idImage'].'\').html(\'<img src=lib/affiche_image.php?provenance=paragraphe&numImg='.$oImg->bgimg_cdn.'>\');'.
               'parent.$(\'#'.$_GET['idFile'].'\').html(\'<input type=hidden name=id_bgimg value='.$oImg->bgimg_cdn.' />\');'.
