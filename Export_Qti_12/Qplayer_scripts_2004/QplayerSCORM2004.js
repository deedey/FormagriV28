...
version 1.0
SCORM 1.3 2004 (www.adlnet.com) conformant
www-e-teach.ch
    ...
    if(SCORMinit){
		...
		if(score=="" || score==null || score==" " || score=="undefined"){
			...
			if(score>scoreMax){
				...
				doSetValue("cmi.score.max",score);
			}
			if (score<=scoreMin){
			  	if(debug){alert("stockScoreMin "+"score :"+score+"\rscoreMin :"+scoreMin);}
			  	doSetValue("cmi.score.min",score);
			}
			...
			if(quizAttempts!="" && quizAttempts!=null && quizAttempts!=0){
				...
				doSetValue("cmi.suspend_data","QA"+quizAttempts+";");
			}
			//traitement du temps au format PTnHnMnS
			
			
			var indexh=temps.indexOf(":");
			var heure=temps.slice(0,indexh);
			temps=temps.slice(indexh+1,temps.length);
			alert(temps);
			var indext=temps.indexOf(":");
			var minute=temps.slice(0,indext);
			temps=temps.slice(indext+1,temps.length);
			alert(temps);
			var seconde=temps;
			
			
			alert(heure+"_"+minute+"_"+seconde);
			var tempsISO8601="PT"+heure+"H"+minute+"M"+seconde+"S";
			alert("tempsISO8601 "+tempsISO8601);
			doSetValue("cmi.session_time",tempsISO8601);
		 	 doTerminate();
...
--------------------------------------------------------------------------------------------------------------*/  
 	function getValue(){
		if(SCORMinit){
			scoreOld=doGetValue("cmi.score.raw");
			scoreMax=doGetValue("cmi.score.max");
			scoreMin=doGetValue("cmi.score.min");
