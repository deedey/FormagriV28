...
class CASClient
{
	...
	function getServerBaseURL()
	{
		...
		{
			$this->_server['base_url'] = 'https://' .			$this->getServerHostname() .			':' .			$this->getServerPort() .			$this->getServerURI();
		}
	...
	function CASClient($server_version, $proxy, $server_hostname, $server_port, $server_uri, $start_session = true)
	{
		...
		else
		{
			...
			// ill-formed ticket, halt
				...
				if (!empty ($ticket))
				{
					phpCAS :: error('ill-formed ticket found in the URL (ticket=`' . htmlentities($ticket,ENT_QUOTES,'ISO-8859-1') . '\')');
				}
	...
	function validateST($validate_url, & $text_response, & $tree_response)
	{
		...
		if (!$this->readURL($validate_url, '' /*cookies*/
			, $headers, $text_response, $err_msg))
		{
			phpCAS :: trace('could not open URL \'' . $validate_url . '\' to validate (' . $err_msg . ')');
			$this->authError('ST not validated', $validate_url, TRUE			/*$no_response*/
			);
		}
		...
		switch ($this->getServerVersion())
		{
			case CAS_VERSION_1_0 :
				if (preg_match('/^no\n/', $text_response))
				{
					...
					$this->authError('ST not validated', $validate_url, FALSE					/*$no_response*/
					, FALSE					/*$bad_response*/
					, $text_response);
				}
				if (!preg_match('/^yes\n/', $text_response))
			...
			case CAS_VERSION_2_0 :
				...
				if (sizeof($success_elements = $tree_response->get_elements_by_tagname("authenticationSuccess")) != 0)
				{
					...
					if (sizeof($failure_elements = $tree_response->get_elements_by_tagname("authenticationFailure")) != 0)
					{
						...
						$this->authError('ST not validated', $validate_url, FALSE						/*$no_response*/
						, FALSE						/*$bad_response*/
						, $text_response, $failure_elements[0]->get_attribute('code')						/*$err_code*/
	...
	function validatePGT(& $validate_url, $text_response, $tree_response)
	{
		...
		if (sizeof($arr = $tree_response->get_elements_by_tagname("proxyGrantingTicket")) == 0)
		{
			...
			$this->authError('Ticket validated but no PGT Iou transmitted', $validate_url, FALSE			/*$no_response*/
			, FALSE			/*$bad_response*/
			, $text_response);
		}
		else
		{
			...
			if ($pgt == FALSE)
			{
				...
				$this->authError('PGT Iou was transmitted but PGT could not be retrieved', $validate_url, FALSE				/*$no_response*/
				, FALSE				/*$bad_response*/
				, $text_response);
			}
			$this->setPGT($pgt);
	...
	function retrievePT($target_service, & $err_code, & $err_msg)
	{
		...
		// open and read the URL
		if (!$this->readURL($cas_url, '' /*cookies*/
		...
		if (!$bad_response)
		{
			...
			// look for a proxyFailure tag
				...
				if (sizeof($arr = $root->get_elements_by_tagname("proxyFailure")) != 0)
				{
					...
					$err_msg = 'PT retrieving failed (code=`' .					$arr[0]->get_attribute('code') .					'\', message=`' .					trim($arr[0]->get_content()) .					'\')';
					phpCAS :: traceEnd(FALSE);
					return FALSE;
				}
				else
	...
	function serviceWeb($url, & $err_code, & $output)
	{
		...
		else
		{
			...
			if (!$this->readURL($service_url, $cookies, $headers, $output, $err_msg))
			{
				...
				$output = sprintf($this->getString(CAS_STR_SERVICE_UNAVAILABLE), $service_url, $err_msg);
				$res = FALSE;
			}
			else
			{
	...
	function serviceMail($url, $flags, & $err_code, & $err_msg, & $pt)
	{
		...
		else
		{
			...
			if (!$stream)
			{
				...
				$err_code = PHPCAS_SERVICE_NOT_AVAILABLE;
				// give an error message
				$err_msg = sprintf($this->getString(CAS_STR_SERVICE_UNAVAILABLE), $service_url, var_export(imap_errors(), TRUE));
				$pt = FALSE;
				$stream = FALSE;
			}
			else
	...
	function validatePT(& $validate_url, & $text_response, & $tree_response)
	{
		...
		{
			phpCAS :: trace('could not open URL \'' . $validate_url . '\' to validate (' . $err_msg . ')');
			$this->authError('PT not validated', $validate_url, TRUE			/*$no_response*/
			);
		}
		// read the response of the CAS server into a DOM object
		if (!($dom = domxml_open_mem($text_response)))
		{
			// read failed
			$this->authError('PT not validated', $alidate_url, FALSE			/*$no_response*/
			, TRUE			/*$bad_response*/
			, $text_response);
		}
		// read the root node of the XML tree
		...
		{
			// read failed
			$this->authError('PT not validated', $validate_url, FALSE			/*$no_response*/
			, TRUE			/*$bad_response*/
			, $text_response);
		}
		// insure that tag name is 'serviceResponse'
		...
		if (sizeof($arr = $tree_response->get_elements_by_tagname("authenticationSuccess")) != 0)
		{
			...
			if (sizeof($arr = $tree_response->get_elements_by_tagname("authenticationFailure")) != 0)
			{
				...
				$this->authError('PT not validated', $validate_url, FALSE				/*$no_response*/
				, FALSE				/*$bad_response*/
				, $text_response, $arr[0]->get_attribute('code')				/*$err_code*/
